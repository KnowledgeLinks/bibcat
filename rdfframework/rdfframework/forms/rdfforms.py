...
class Form(flask_wtf.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.is_subobj:
            ...
            x=1
        #if self.data_subject_uri is not None:
            #self._get_form_data()
...
def rdf_framework_form_factory(form_url, **kwargs):
    ...
    instructions = get_form_instructions_json(_app_form.get('kds_formInstructions'), \
            _instance)
    _lookup_class_uri = kwargs.get("classUri", instructions.get("kds_lookupClassUri"))
    _lookup_prop_uri = kwargs.get("propUri", instructions.get("kds_lookupPropUri"))
    ...
    for fld in fields:
        ...
        if field:
            form_field = get_wtform_field(field, _instance, **kwargs)
            if isinstance(form_field, list):
                ...
                for nfld in form_field:
                    ...
                    if nfld.get('kds_field'):
                        ...
                        _new_field['kds_formFieldOrder'] = float(_new_field['kds_formFieldOrder']) + i
                        if fld.get("doNotSave"):
                            _new_field['doNotSave'] = True
                        else:
                            _new_field['doNotSave'] = False
                        augmented_field = add_field_attributes(nfld['kds_field'],_new_field)
    ...
    if kwargs.get("is_subobj"):
        field = {'kds_formFieldName':'subjectUri',
                 ...
                 'kds_formLabelName':'dataSubjectUri',
