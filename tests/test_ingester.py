...
WHERE {{
    ?bnode a <{0}> .
        ...
        results = [row for row in self.ingester.graph.query(
            sparql.format(ingesters.BF.GenerationProcess))]
...
}}""".format(ingesters.BF.generationDate)
        self.ingester.add_admin_metadata(self.entity)
...
class TestRemoveBlankNodes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ingester.graph.add((self.bnode, rdflib.RDF.type, ingesters.BF.Isbn))
    ...
    def test_simple_replace_uri(self):
        self.assertEqual(
            self.ingester.graph.value(
                predicate=ingesters.BF.instanceOf,
